...
import EditCInfoForm from './EditCInfoForm';
import intl from 'react-intl-universal';
class EditCInfoModal extends Component {
    constructor(props) {
        super(props);
        this.state = {
            spinning: false,
    ...
    render() {
        ...
        return (
            ...
            <Modal
                title={this.props.mode === 'add' ? '添加经销商' : '编辑经销商'}
    ...
    handleOk = () => {
        ...
        let isValid = false;
        form.validateFields(['agent_keyid', 'agent_name', 'agent_owner', 'agent_phone', 'agent_locat'], { force: true }, (err, value) => {
            isValid = isNull(err);
        });
        //校验失败直接返回
        if (!isValid) {
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        //     //触发搜索
        //     dispatch(agentCtors.getDoSearchAgentAction(type, value,admtp, pageindex, pagesize));
...
export default connect(mapStateToProps, mapDispatchToProps)(EditCInfoModal);